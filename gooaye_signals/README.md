...
2. 編輯那一個檔：`id`(=檔名)、門檻常數、`bindings`(資料源)、純函式 `_compute`、
   四個 `interpretations`、`purpose`/`track`/`shape`/`order`/`cluster`/`widget`/`tags`/`in_master`。
...
## 每個訊號都回答「四個問題」——直接顯示在卡片上

這是使用者的核心需求，是 `SignalSpec` 的**必填欄位**（Phase 1 測試強制檢查），且**全部渲染在網頁卡片上**：

1. **驗證什麼 (`purpose`)**：一句話說清楚這張卡存在的目的 → 卡片收合時也固定顯示在名稱下方。
2. **追什麼 (`track`)**：白話「實際盯哪一份資料、為什麼重要」→ 展開後的〔追什麼〕列。
3. **變化的長相 (`shape`)**：實際圖表 + 白話「動起來長什麼樣」→ 圖表本體 + 圖下方的〔怎麼看〕列。
4. **各狀態含義 (`interpretations`)**：🟢🟡🔴 三燈對照**全部列出**、當前燈高亮並標〔現在〕→ 卡片底部燈號對照區。
...
「供需失衡早期預警」。四張早期預警設 `featured=True`，會在三張主燈卡後**直接顯示**，
不再藏進摺疊的補充面板；但仍不計主燈。毛利率目前是單一公司同口徑代理、CSP 尚有公司
待補、現貨／合約價差仍在建立同品項月度基準、CXMT 只能把計畫與量產分開追；未達多來源、
連續資料的門檻前，不用它們悄悄改變既有主燈。

#### 記憶體供需反轉早期預警（使用者提供影片的四條新增訊號）
...
cluster 2 與 cluster 3 在資料與主燈上仍各自獨立，但前端共同包在「AI 榮景外溢驗證」：
前者看 AI／半導體需求是否外溢到傳統封裝上游，後者看需求是否從三大雲下沉到企業
自建／混合雲。共同問題是「AI 榮景有沒有從少數巨頭真正向外擴散」。
...
3.5→8.4→12.5B 推算開源絕對額——占比降但額仍增＝黃，防分母效應誤讀）。
...
來源優先順序：公司 IR／財報、監管機關或交易所公告是一手依據；TrendForce／
DRAMeXchange 是價格序列的專業原始資料；Reuters／AP 等僅用來交叉核對尚無一手數字的
事件。缺正式資料、公司缺項或不足兩筆可比快照時，一律留黃／灰，不用媒體推測補成結論。
