...
| --- | --- | --- |
| 2026-07-19 12:59 | 827d06d | feat(mrt)：補齊全台現行捷運——三鶯線(試營運)、淡海輕軌綠山/藍海線、安坑輕軌、台中捷運綠線；新增台中分頁，全網 18 線 258 站 |
| 2026-07-19 09:08 | a247ab0 | ci(fix)：移除捷運圖 Pages workflow——Pages 還給 gooaye_signals，捷運圖改走獨立管道（githack／待開專屬 repo） |
| 2026-07-19 09:06 | ad0c007 | ci(fix)：一次性還原 gooaye_signals Pages（捷運圖部署誤蓋站台），從 master 重建＋抓新資料部署成功 |
| 2026-07-19 08:58 | 604f9bf | ci(mrt)：GitHub Pages 自動部署 workflow，網站上線（後發現誤蓋 gooaye_signals 站台，已於 09:06 還原、09:08 移除） |
| 2026-07-19 01:05 | 5e23d3a | feat(mrt)：路網圖式示意排版（45°/90°折線+折點引擎）＋列車標記改膠囊車廂+方向箭頭＋完整 HTML 供公開網址（raw.githack）瀏覽 |
| 2026-07-18 22:59 | 3d78b2b | feat(mrt)：列車標記改水滴形導航箭頭——尖端隨行進方向旋轉，停站時預指離站方向 |
| 2026-07-18 22:56 | 956c1bc | feat(mrt)：台灣捷運動態圖網站（taiwan_mrt_tracker/）——北北桃+高雄 10 線、依班距即時模擬列車位置，已發佈 Artifact 供手機瀏覽 |
| 2026-07-15 23:30 | 265acaa | fix：自審四修——事件簿綠燈加「具名背書」門檻（供應側湊不出綠）、訂單卡補資料至、籃子卡揭露 Dell 稀釋、事件簿腰斬措辭與 Menlo 卡對齊 |
| 2026-07-15 23:05 | ff0a8c0 | feat：Menlo 卡升級「風向×體量」——補分母（總支出 3.5→8.4→12.5B）算出推算開源額（0.67→1.09→1.38B），雙變數真值表，燈紅→黃（占比降但額仍增） |
| 2026-07-15 22:40 | cdde8c9 | fix：Menlo 卡改「風向計」框架——使用者指正分母效應（占比降≠用量減），文案明示只答「新增的錢流向哪邊」 |
| 2026-07-15 22:10 | 38e3f07 | feat：cluster 3「地端/混合雲 AI」——DELL+HPE 籃＋HPE 訂單動能（手動季更）＋事件簿＋Menlo 開源占比；新增 manual_series 資料源（repo 內 JSON、逐點附出處） |
| 2026-07-12 21:20 | 6b37319 | docs：分工鐵律入 CLAUDE.md/HANDOFF.md——口述訊號→忠實實作為止，不附投資判斷/下單建議 |
| 2026-07-12 21:05 | c69b949 | fix：營收綠燈加水位守門（未連降「且」YoY>0 才綠，年減中降黃）——修「衰退收斂卻亮綠說擴張」邏輯洞；四雄表補回「資料至」 |
| 2026-07-12 20:45 | f22415b | feat：營收卡升級四雄動能表（每家動能燈＋近12月YoY走勢＋晚報標示，真值表彙總）；黃燈標語改「動能鈍化或分歧」 |
| 2026-07-12 20:25 | c1fe08e | feat(web)：主題區塊層級改版——區塊容器＋大標題（色點/粗體/狀態膠囊）＋補充面板托盤化，大小標題三級距 |
